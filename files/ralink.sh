...
#		config_get hidessid $vif hidden 0	
		...
		[ "$mode" == "ap" ] && {
			...
			#apcli_channel=$(iwpriv ra0 set SiteSurvey=1 && iwpriv ra0 get_site_survey | grep "$ApCliSsid" | head -n 1 | cut -d ' ' -f1)
		  apcli_channel=$(iwpriv ra0 set SiteSurvey=1 && iwpriv ra0 get_site_survey | grep "$ApCliSsid" | awk '{gsub(ApCliSsid, "SSID", $0); printf("%s\n", $0);}' ApCliSsid="$ApCliSsid" | sort -nr -k5 | head -n 1 | cut -d ' ' -f1)   #chy
			...
			[ -n $apcli_channel -a "$channel" == "auto" ] && channel=$apcli_channel
...
config wifi-iface
	...
	option network	lan
	option mode     ap
	option ssid     IntoRobot-Atom${i#0}$(cat /sys/class/net/eth0/address|awk -F ":" '{print $5""$6 }'| tr a-z A-Z)
	option encryption psk2
	option key intorobot
	option ApCliEnable '1'
	option ApCliSsid 'SSID_USER'
	option ApCliAuthMode 'WPA2PSK'
	option ApCliEncrypType 'AES'
	option ApCliPassWord 'PASSWORD_USER'
EOF
	done
}
